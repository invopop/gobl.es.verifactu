...
The two modalities report faults under a namespace each, so a fault makes clear
which set of rules rejected the document:

| Namespace | Rules | Example |
| --- | --- | --- |
| `GOBL-ES-VERIFACTU` | Invoices and tax combos | `GOBL-ES-VERIFACTU-BILL-INVOICE-02` |
| `GOBL-ES-NOVERIFACTU` | Status events and their complements | `GOBL-ES-NOVERIFACTU-BILL-STATUS-01` |

The NO VERI*FACTU rules previously reported under a bare `NOVERIFACTU`
namespace, from a rule set held outside the addon that only this library
invoked. The set and assertion parts are unchanged, so `NOVERIFACTU-BILL-STATUS-01`
is now `GOBL-ES-NOVERIFACTU-BILL-STATUS-01`. Anything matching on the old codes
needs updating.
