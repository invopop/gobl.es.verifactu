...
func TestInvoiceDocumentScenarios(t *testing.T) {
	t.Run("with addon", func(t *testing.T) {
		...
		assert.Equal(t, i.Tax.Ext.Get(addon.ExtKeyDocType).String(), "F1")
	})
	...
	t.Run("simplified invoice", func(t *testing.T) {
		...
		assert.Equal(t, i.Tax.Ext.Get(addon.ExtKeyDocType).String(), "F2")
	})

	t.Run("credit note", func(t *testing.T) {
		i := testInvoiceStandard(t)
		require.NoError(t, i.Correct(bill.Credit, bill.WithExtension(addon.ExtKeyDocType, "R1")))
		// require.NoError(t, i.Calculate())
		assert.Len(t, i.Notes, 0)
		assert.Equal(t, i.Tax.Ext.Get(addon.ExtKeyDocType).String(), "R1")
		assert.Equal(t, i.Tax.Ext.Get(addon.ExtKeyCorrectionType).String(), "I")
	})

	t.Run("corrective", func(t *testing.T) {
		i := testInvoiceStandard(t)
		require.NoError(t, i.Correct(bill.Corrective, bill.WithExtension(addon.ExtKeyDocType, "R2")))
		assert.Len(t, i.Notes, 0)
		assert.Equal(t, i.Tax.Ext.Get(addon.ExtKeyDocType).String(), "R2")
		assert.Equal(t, i.Tax.Ext.Get(addon.ExtKeyCorrectionType).String(), "S")
	})
	...
	t.Run("corrective special", func(t *testing.T) {
		...
		require.NoError(t, i.Calculate())
		require.NoError(t, i.Correct(bill.Corrective, bill.WithExtension(addon.ExtKeyDocType, "R2")))
		assert.Equal(t, "R2", i.Tax.Ext.Get(addon.ExtKeyDocType).String())
	})
	...
	t.Run("simplified corrective invoice", func(t *testing.T) {
		...
		assert.Equal(t, i.Tax.Ext.Get(addon.ExtKeyDocType).String(), "R5")
	})
...
func TestCorrectionNormalize(t *testing.T) {
	...
	t.Run("credit note routes doc-type to invoice", func(t *testing.T) {
		i := testInvoiceStandard(t)
		require.NoError(t, i.Calculate())
		assert.Equal(t, "F1", i.Tax.Ext.Get(addon.ExtKeyDocType).String())

		require.NoError(t, i.Correct(bill.Credit, bill.WithExtension(addon.ExtKeyDocType, "R1")))

		assert.Equal(t, "R1", i.Tax.Ext.Get(addon.ExtKeyDocType).String())
		// Doc type should not remain on preceding
		assert.Empty(t, i.Preceding[0].Ext.Get(addon.ExtKeyDocType))
	})
}
