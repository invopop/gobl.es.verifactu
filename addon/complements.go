...
// SchemaBase is the base URL for all verifactu complement schemas.
const SchemaBase schema.ID = "https://verifactu.gobl.org/v0"

func init() {
	schema.Register(SchemaBase,
...
// InvoiceAnomalyLaunch contains the results of launching an anomaly detection
// process on invoice records. Used with event type 03.
...
// InvoiceAnomaly describes a detected anomaly in invoice records. Used with
// event type 04.
...
// EventAnomalyLaunch contains the results of launching an anomaly detection
// process on event records. Used with event type 05.
...
// EventAnomaly describes a detected anomaly in event records. Used with
// event type 06.
...
// InvoiceExport contains summary data for an export of invoice records over a
// period. Used with event type 08.
...
// EventExport contains summary data for an export of event records over a period.
// Used with event type 09.
...
// EventSummary provides a summary of events and invoices over a period. Used with
// event type 10.
...
var complementRules = []*rules.Set{
	invoiceAnomalyLaunchRuleSet(),
	invoiceAnomalyRuleSet(),
	eventAnomalyLaunchRuleSet(),
	eventAnomalyRuleSet(),
	invoiceExportRuleSet(),
	eventExportRuleSet(),
	eventSummaryRuleSet(),
}

func invoiceAnomalyLaunchRuleSet() *rules.Set {
...
func invoiceAnomalyRuleSet() *rules.Set {
	return rules.For(new(InvoiceAnomaly),
		...
		rules.Field("invoice",
			rules.Field("issuer_tax_code",
				rules.Assert("02", "issuer tax code is required", is.Present),
			),
			rules.Field("code",
				rules.Assert("03", "invoice code is required", is.Present),
			),
		),
	)
}

func eventAnomalyLaunchRuleSet() *rules.Set {
...
func eventAnomalyRuleSet() *rules.Set {
	return rules.For(new(EventAnomaly),
		...
		rules.Field("event",
			...
			rules.Field("timestamp",
				rules.Assert("03", "timestamp is required", is.Present),
			),
			rules.Field("fingerprint",
				rules.Assert("04", "fingerprint is required", is.Present),
			),
		),
	)
}

func invoiceExportRuleSet() *rules.Set {
...
func eventExportRuleSet() *rules.Set {
...
func eventSummaryRuleSet() *rules.Set {
