...
func TestInvoicePartyNormalization(t *testing.T) {
	...
	t.Run("customer with nil identity followed by valid identity", func(t *testing.T) {
		...
		assert.Equal(t, addon.ExtCodeIdentityTypePassport, inv.Customer.Identities[0].Ext.Get(addon.ExtKeyIdentityType))
	})
	...
	t.Run("foreign identity normalization", func(t *testing.T) {
		...
		require.NoError(t, inv.Calculate())
		assert.Equal(t, addon.ExtCodeIdentityTypeForeign, inv.Customer.Identities[0].Ext.Get(addon.ExtKeyIdentityType))
	})
	...
	t.Run("resident identity normalization", func(t *testing.T) {
		...
		require.NoError(t, inv.Calculate())
		assert.Equal(t, addon.ExtCodeIdentityTypeResident, inv.Customer.Identities[0].Ext.Get(addon.ExtKeyIdentityType))
	})
	...
	t.Run("other identity normalization", func(t *testing.T) {
		...
		require.NoError(t, inv.Calculate())
		assert.Equal(t, addon.ExtCodeIdentityTypeOther, inv.Customer.Identities[0].Ext.Get(addon.ExtKeyIdentityType))
	})
	...
	t.Run("self-billed", func(t *testing.T) {
		...
		require.NoError(t, inv.Calculate())
		assert.Equal(t, addon.ExtCodeIssuerTypeCustomer, inv.Tax.Ext.Get(addon.ExtKeyIssuerType))
	})
	...
	t.Run("with issuer", func(t *testing.T) {
		...
		require.NoError(t, inv.Calculate())
		assert.Equal(t, addon.ExtCodeIssuerTypeThirdParty, inv.Tax.Ext.Get(addon.ExtKeyIssuerType))
	})
}
...
func TestInvoiceValidation(t *testing.T) {
	t.Run("standard invoice", func(t *testing.T) {
		...
		assert.Equal(t, inv.Tax.Ext.Get(addon.ExtKeyDocType).String(), "F1")
	})
	...
	t.Run("simplified invoice", func(t *testing.T) {
		...
		require.NoError(t, inv.Calculate())
		require.NoError(t, rules.Validate(inv))
		assert.Equal(t, inv.Tax.Ext.Get(addon.ExtKeyDocType).String(), "F2")
	})
	...
	t.Run("simplified substitution without customer", func(t *testing.T) {
		...
		inv.Customer = nil
		require.NoError(t, inv.Calculate())
		assert.Equal(t, "F2", inv.Tax.Ext.Get(addon.ExtKeyDocType).String())

		require.NoError(t, inv.Correct(bill.Corrective, bill.WithCopyTax()))
		require.NoError(t, rules.Validate(inv))
		// Should get R5 for simplified corrective
		assert.Equal(t, "R5", inv.Tax.Ext.Get(addon.ExtKeyDocType).String())
		assert.Equal(t, "S", inv.Tax.Ext.Get(addon.ExtKeyCorrectionType).String())
	})
	...
	t.Run("corrective invoice with preceding", func(t *testing.T) {
		...
		t.Log(string(data))
		require.NoError(t, rules.Validate(inv))
		assert.Equal(t, "R1", inv.Tax.Ext.Get(addon.ExtKeyDocType).String())
	...
	t.Run("simplified substitution with customer without tax ID", func(t *testing.T) {
		...
		require.NoError(t, inv.Calculate())
		require.NoError(t, rules.Validate(inv))
		assert.Equal(t, inv.Tax.Ext.Get(addon.ExtKeyDocType).String(), "R5")
	})
